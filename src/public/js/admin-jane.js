const API_BASE = '/api';
const BANK_CODE = 'jane';

let currentToken = localStorage.getItem('jane_admin_token');
let currentFilter = null;

if (!currentToken) {
    window.location.href = '/admin-login-jane';
}
...
async function apiRequest(endpoint, method, data = null) {
    const token = localStorage.getItem('jane_admin_token');  // ← CORRIGIDO
    ...
    const res = await fetch(`${API_BASE}${endpoint}`, {
        ...
        headers: { 
            ...
            'Authorization': 'Bearer ' + token  // ← USA jane_admin_token
        },
    ...
    if (res.status === 401 || res.status === 403) {
        localStorage.removeItem('jane_admin_token');
        window.location.href = '/admin-login-jane';
...
async function loadLoansData(accountId, startDate = '', endDate = '') {
    try {
        ...
        loans.forEach(l => {
            // Garantir que saldo devedor nunca seja negativo
            let saldoDevedor = l.remaining_balance;
            ...
            html += `
                <div style="background: #f8fafc; padding: 15px; border-radius: 12px; margin-bottom: 15px;">
                    ...
                    <p><strong>Saldo Devedor:</strong> ${saldoDevedor.toLocaleString('pt-CV')} CVE</p>
...
document.getElementById('viewScheduledBtn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        html += '<button class="action-btn block" onclick="adminCancelAllSchedules()" style="font-size:11px;">🗑️ Cancelar Todos Pendentes</button>';
        html += '</div>';
        
        html += '<table style="width:100%; font-size:12px; border-collapse:collapse;">';
        html += '<thead><tr style="background:#f8fafc;">';
        html += '<th>Status</th><th>Cliente</th><th>Data</th><th>Valor</th><th>Destino</th><th>Modo</th><th>Ação</th>';
        html += '</tr></thead><tbody>';
        
        schedules.forEach(s => {
            const statusIcon = s.status === 'pending' ? '⏳' : s.status === 'executed' ? '✅' : s.status === 'cancelled' ? '❌' : '⚠️';
            const statusColor = s.status === 'pending' ? '#3b82f6' : s.status === 'executed' ? '#10b981' : '#ef4444';
            
            html += '<tr>';
            html += `<td style="color:${statusColor};">${statusIcon} ${s.status}</td>`;
            html += `<td>${s.from_name}</td>`;
            html += `<td>${new Date(s.scheduled_date + ' UTC').toLocaleString('pt-PT')}</td>`;
            html += `<td>${s.amount.toLocaleString('pt-CV')} CVE</td>`;
            html += `<td>${s.to_bank.toUpperCase()}: ${s.to_account_number}</td>`;
            html += `<td>${s.mode === 'recurring' ? '🔄' : '📌'} ${s.mode === 'recurring' ? s.frequency || '' : 'Pontual'}</td>`;
            
            if (s.status === 'pending') {
                html += `<td><button class="action-btn block" onclick="adminCancelSchedule(${s.id})" style="font-size:11px;">Cancelar</button></td>`;
            } else {
                html += '<td>-</td>';
            }
            html += '</tr>';
