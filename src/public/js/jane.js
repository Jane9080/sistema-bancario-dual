    ...
    try {
        ...
        if (transferType === 'same') {
            ...
            result = await apiRequest(`/transfer/inter/${BANK_CODE}`, 'POST', {
                to_bank: 'kent', to_account: destinationAccount, amount, description, confirm_key: confirmKey
            });
        }
        if (result.success) {
...
async function downloadReceipt(receiptId) {
    try {
        window.open(`/api/receipts/${BANK_CODE}/download/${receiptId}`, '_blank');
    } catch (error) {
        showToast(error.message, 'error');
    }
}
    ...
    if (transferTypeSelect && feeInfo) {
        transferTypeSelect.addEventListener('change', () => {
            if (transferTypeSelect.value === 'same') {
                ...
                feeInfo.innerHTML = '<i class="fas fa-info-circle"></i> Taxa: 150 CVE (Jane → Kent)';
